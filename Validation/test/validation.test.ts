...
describe("Validation Testing", () => {
  ...
  /* Custom Validation
    Custom Validation menggunakan method transform yang mana parameternya adalah function untuk 
    custom validation tersebut.
  */
  const mustUpperCase = (data: string, ctx: z.RefinementCtx) => {
    if (data != data.toUpperCase()) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "must be in upper case",
      });
      return z.NEVER;
    } else {
      return data;
  ...
  it("Should can support custom validation", () => {
    const loginSchema = z.object({
    ...
    const req = {
        username: "EMAIL@EXAMPLE.COM", //username harus uppercase sesuai ketentuan transform
        password: "123467890",
    }
    const res = loginSchema.parse(req)
    console.log(res)
  });
});
